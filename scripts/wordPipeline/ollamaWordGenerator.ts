...
/**
 * Generate words using Ollama mistral-small3.2. Returns items shaped for Firestore (languages/regions arrays).
 */
...
/**
 * Phase 1 only: one Ollama call → 1 word per category (19 words). Fast; use for mini so user can redirect.
 */
 ...
 * First call returns quickly so UI can redirect; full run continues in background.
 */
...
/**
 * Legacy: 20 words per category via 19 sequential calls (no phase 1). Prefer generateWordsForAllCategoriesTwoPhase for UI.
 */
...
async function main() {
  ...
  const input: GenerateWordsForAllCategoriesInput = {
    ...
    console.error(
      `Generating words for all categories (${input.countPerCategory ?? DEFAULT_COUNT_PER_CATEGORY} per category) with Ollama (${MODEL})...`,
